...
export default function FunnelPage() {
  ...
  const handleFindLink = async () => {
    if (!findLinkEmail || !findLinkEmail.includes('@')) {
      setFindLinkMessage('Please enter a valid email address.')
      return
    }
    setFindLinkLoading(true)
    setFindLinkMessage('')
    try {
      const res = await fetch('/api/find-link', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: findLinkEmail }),
      })
  ...
  return (
    <>
          ...
          <div style={{ width:'100%', maxWidth:'480px' }}>
            ...
            {screen === 1 && (
              <div style={screenStyle}>
                ...
                <button disabled={!agreed} onClick={() => goTo(2)} style={{...ageBtnPrimaryStyle, opacity: agreed ? 1 : 0.5, cursor: agreed ? 'pointer' : 'not-allowed'}}>
      ...
      {screen === 5 && (
        <div style={{ width:'100%', maxWidth:'420px', margin:'0 auto', background:'var(--cream)', border:'1px solid var(--border)', borderRadius:'20px', overflow:'hidden' }}>
          ...
          <nav style={{background:'var(--black)',padding:'18px 20px',display:'flex',alignItems:'center',justifyContent:'space-between',borderRadius:'20px 20px 0 0'}}>
            ...
            <button onClick={scrollToJoin} style={{background:'var(--cream)',color:'var(--black)',fontSize:'10px',fontWeight:700,letterSpacing:'0.08em',textTransform:'uppercase',padding:'9px 16px',borderRadius:'40px',border:'none',cursor:'pointer'}}>Join The Journey</button>
          </nav>
          ...
          <div style={{background:'var(--black)',padding:'24px 20px 8px'}}>
            <p style={{fontFamily:"'Cormorant Garamond', serif",fontSize:'28px',fontWeight:700,lineHeight:1.2,color:'var(--white)'}}>
          ...
          <div style={{background:'var(--black)',padding:'28px 20px 32px'}}>
            ...
            <div style={{width:'100%',aspectRatio:'3/4.2',borderRadius:'16px',overflow:'hidden',margin:'28px 0 18px',background:'var(--warm)'}}>
              <img src="/images/hero.png" alt="OI Body Chemistry" style={{width:'100%',height:'100%',objectFit:'cover',objectPosition:'top center'}} />
            </div>
            ...
            <div style={{background:'var(--cream)',padding:'18px 12px',display:'grid',gridTemplateColumns:'repeat(2,1fr)',gap:'16px',margin:'0 -20px -32px'}}>
              {[{icon:'lock',label:'Private Community Access'},{icon:'pkg',label:'Discreet Shipping'},{icon:'x-circle',label:'Cancel Anytime'},{icon:'shield',label:'Secure & Private'}].map(t => (
          ...
          <div style={{padding:'56px 20px 32px',background:'var(--black)',color:'var(--white)'}}>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:'16px'}}>
              {[
                {num:'01', title:'Know Your Blood Type', desc:'Your blood type determines how your body processes food, stores fat, and responds to supplements. This is where your reset starts.', pdf:'/pdfs/blood_type_guide.pdf', dl:'Know_Your_Blood_Type_Reset_Guide.pdf', video:'/videos/blood_type.mp4', cover:'/videos/covers/blood_type_cover.jpg'},
                {num:'02', title:'Deficiencies & What Tests You Need', desc:"Most women are deficient and don't know it. We break down which tests to request and what your results mean for your transformation.", pdf:'/pdfs/testing_guide.pdf', dl:'Deficiencies_And_Tests_Guide.pdf', video:'/videos/testing.mp4', cover:'/videos/covers/testing_cover.jpg'},
                {num:'03', title:'Body Functions + Peptides & Herbs', desc:'Understand how your metabolism, liver, and gut work together — plus peptides and herbs that can amplify your results.', pdf:'/pdfs/body_functions_guide.pdf', dl:'Body_Functions_Guide.pdf', video:'/videos/body_functions.mp4', cover:'/videos/covers/body_functions_cover.jpg'},
                {num:'04', title:'Your First Order & Why It Works', desc:"See what's in your first order, how the booster works, and the protocol to follow while you wait on test results.", pdf:'/pdfs/protocol_guide.pdf', dl:'Your_First_Order_And_Why_It_Works.pdf', video:'/videos/first_order.mp4', cover:'/videos/covers/first_order_cover.jpg'},
              ].map(v => (
                <div key={v.num} style={{background:'var(--warm)',border:'1px solid var(--border)',borderRadius:'12px',padding:'16px'}}>
                  {v.video ? (
                    <video controls poster={v.cover} style={{width:'100%',aspectRatio:'16/9',borderRadius:'8px',marginBottom:'14px',background:'#000'}}>
                      <source src={v.video} type="video/mp4" />
                    </video>
                  ) : (
                    <div style={{width:'100%',aspectRatio:'16/9',borderRadius:'8px',backgroundImage:v.cover?`linear-gradient(rgba(5,5,5,0.55),rgba(5,5,5,0.55)), url(${v.cover})`:'linear-gradient(160deg,#1a1715 0%,#0d0b09 100%)',backgroundSize:'cover',backgroundPosition:'center',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:'8px',marginBottom:'14px',border:'1px solid var(--border)'}}>
          ...
          <div style={{padding:'56px 20px 32px',background:'var(--black)',color:'var(--white)'}}>
            ...
            <div style={{width:'100%',aspectRatio:'1/1',borderRadius:'12px',overflow:'hidden',marginBottom:'20px'}}>
              <img src="/images/product.png" alt="OI Body Chemistry product" style={{width:'100%',height:'100%',objectFit:'cover',objectPosition:'center',display:'block'}} />
            </div>
            {[
              ...
              <div key={p.name} style={{border:'1px solid var(--border)',borderRadius:'12px',padding:'18px',marginBottom:'14px'}}>
                ...
                <button onClick={() => { setFormData(f => ({...f, booster: p.name})); scrollToJoin(); }} style={{display:'block',width:'100%',background:'var(--gold)',color:'var(--black)',fontSize:'11px',fontWeight:700,letterSpacing:'0.1em',textTransform:'uppercase',textAlign:'center',padding:'12px',borderRadius:'6px',border:'none',cursor:'pointer'}}>Start With {p.name}</button>
              </div>
            ))}
          </div>
      ...
      {privacyOpen && <Modal title="Privacy Policy" onClose={() => setPrivacyOpen(false)}>
        ...
        <h3 style={modalH3Style}>6. Sharing</h3><p>We do not sell your personal information. We may share with trusted service providers only as necessary.</p>
        <h3 style={modalH3Style}>7. Children Privacy</h3><p>Our services are intended for individuals 18 years of age or older.</p>
      ...
      {termsOpen && <Modal title="Terms of Service" onClose={() => setTermsOpen(false)}>
        ...
        <h3 style={modalH3Style}>7. Pricing and Payment</h3><p>All prices are in U.S. Dollars. Payment is processed through a secure third-party processor.</p>
        <h3 style={modalH3Style}>8. Shipping</h3><p>We ship to all U.S. states. Orders are not shipped on Sundays.</p>
        <h3 style={modalH3Style}>9. Returns and Refunds</h3><p>All sales are final unless otherwise required by applicable law.</p>
...
const screenStyle = { background:'var(--deep)',border:'1px solid var(--border)',borderRadius:'20px',padding:'56px 40px',textAlign:'center',display:'flex',flexDirection:'column',alignItems:'center',position:'relative',overflow:'hidden',animation:'fadeIn 0.5s ease forwards',color:'var(--white)' }
